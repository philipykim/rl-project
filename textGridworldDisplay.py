...
class TextGridworldDisplay:
  ...
  def displayQValues(self, agent, label, currentState = None, message = None):
    if message != None: 
      print message
...
def prettyPrintQValues(gridWorld, possibleActions, qValues, display=None):
    grid = gridWorld.grid
    maxLen = 11
    newRows = []
    for y in range(grid.height):
      ...
      for x in range(grid.width):
        ...
        if display and state in display:
          ewString = westString + "  " + display[state] + "  " + eastString
